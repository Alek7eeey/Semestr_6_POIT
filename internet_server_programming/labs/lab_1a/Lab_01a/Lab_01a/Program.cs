...
//https://localhost:7027/aaa.KAD?parmA=14&parmB=15
app.MapGet("/{path}.KAD", (string? parmA, string? parmB) =>
{
    if (parmA != null && parmB != null && int.TryParse(parmA, out var a) && int.TryParse(parmB, out var b))
    {
        return $"GET-Http-KAD:ParmA ={parmA},ParmB = {parmB}";
    }

    return "Error during request: invalid params";
}); 

// POST: https://localhost:7027/sdc.KAD?parmA=5&parmB=12
app.MapPost("/{path}.KAD", (string? parmA, string? parmB) =>
{
    if (parmA != null && parmB != null && int.TryParse(parmA, out var a) && int.TryParse(parmB, out var b))
    {
        return $"POST-Http-KAD:ParmA ={parmA},ParmB = {parmB}";
    }

    return "Error during request: invalid params";
});

// PUT: https://localhost:7027/sdc.KAD?parmA=4&parmB=13
app.MapPut("/{path}.KAD", (string? parmA, string? parmB) =>
{
    if (parmA != null && parmB != null && int.TryParse(parmA, out var a) && int.TryParse(parmB, out var b))
    {
        return $"PUT-Http-KAD:ParmA ={parmA},ParmB = {parmB}";
    }

    return "Error during request: invalid params";
});

// POST: https://localhost:7027/KAD/sum?x=2&y=12
app.MapPost("/KAD/sum", (int x, int y) =>
{
    return x + y;
});
