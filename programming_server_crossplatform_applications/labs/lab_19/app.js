...
const homeController = require("./routes/route");
...
app.get("/resource", (req, res) => {
  if (req.payload)
    res.status(200).send(`Resource open! User: ${req.payload.id}-${req.payload.role}`);
  else res.status(401).send("Error: you need to log in");
});
