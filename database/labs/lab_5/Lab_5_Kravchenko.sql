...
UPDATE Отпуска
SET Вид_отпуска = 'Annual'
WHERE Вид_отпуска = 'Ежегодный';

UPDATE Отпуска
SET Вид_отпуска = 'Additional'
WHERE Вид_отпуска = 'Дополнительный';
...
VALUES (1, 1, 3, '2023-07-02', NULL),
       (2, 2, 1, '2023-04-01', NULL),
       (3, 3, 5, '2023-12-01', NULL),
       (4, 4, 2, '2023-11-01', NULL);

INSERT INTO Передвижение_по_отделам (ID, Сотрудник_ID, Отдел_ID, Дата_начала, Дата_окончания)
VALUES (2, 1, 1, '2023-02-02', NULL),
...
FROM
    Сотрудники
JOIN
    Зарплаты ON Сотрудники.ID = Зарплаты.Сотрудник_ID AND Зарплаты.Дата BETWEEN DATEADD(YEAR, -1, GETDATE()) AND GETDATE()
LEFT JOIN
...
(
    ...
    WHERE
        Z.Дата BETWEEN DATEADD(MONTH, -6, GETDATE()) AND GETDATE()
    GROUP BY
