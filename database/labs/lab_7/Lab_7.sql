...
FROM (
    SELECT
        ...
        amount_2022,
        CASE
            WHEN emp_id IN (21, 22) THEN ROUND((max_amount[22] - amount_2022) / 2 + amount_2022, 2)
            ELSE ROUND((max_amount[22] - amount_2022) / 2 + amount_2022, 2)
        END AS plan_amount
    FROM (
        SELECT
            year,
    ...
    MODEL
        PARTITION BY (year, month, prd_type_id)
        DIMENSION BY (emp_id)
        MEASURES (amount_2022)
        IGNORE NAV
        UNIQUE DIMENSION
        RULES (
            max_amount[21] = MAX(amount_2022) WHERE emp_id = 21,
            max_amount[22] = MAX(amount_2022) WHERE emp_id = 22,
            max_amount[23] = MAX(amount_2022) WHERE emp_id = 23,
            max_amount[24] = MAX(amount_2022) WHERE emp_id = 24
        )
)
